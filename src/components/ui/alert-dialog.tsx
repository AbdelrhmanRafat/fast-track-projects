...
function AlertDialog({
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      registerDialog(alertDialogId, closeFn)
      openDialog(alertDialogId)
    } else {
      unregisterDialog(alertDialogId)
    }
    return () => {
      unregisterDialog(alertDialogId)
    }
  }, [isOpen, alertDialogId, registerDialog, unregisterDialog, openDialog, onOpenChange])
...
function AlertDialogHeader({
  ...
  return (
    <div
      ...
      className={cn("flex flex-col gap-2 justify-center items-center text-center", className)}
      {...props}
    />
  )
}
