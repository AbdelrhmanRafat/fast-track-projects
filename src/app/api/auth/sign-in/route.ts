...
import type { SignInResponse } from '@/lib/services/auth/types';

export async function POST(
  request: NextRequest
): Promise<NextResponse<ApiResponse<SignInResponse> | ApiResponse<null>>> {
  try {
    ...
    const response = await networkLayer.post<ApiResponse<SignInResponse>>(
      '/sign-in',
      body
    );
    ...
    const backendMessage = (() => {
      if (!d) return error.message || 'Sign in failed';
